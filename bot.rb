...
$bot = Cinch::Bot.new do
  configure do |c|
    ...
    ENV['TRELLO_ADD_CARDS_LIST'] ||= "Inbox"
...
on :message, /http\:\/\/(.*)\s?/ do |m, n|
  body = Nokogiri::HTML(open('http://'+n))
  m.reply body.css('title')[0].text
end
...
#  m.reply body
#  $link_store[m] ||= []
#  $link_store[m] << { :url => match[0], :nick => nick, :date => Time.now }
#  $link_store[m].shift if $link_store[channel].size > 10
#  
#end

# echo back collected URLs
#on :message, /^\!(links|lionks)/i do |m|
#  if $link_store[m]
#    # msg channel, "Most recent links (#{$link_store[channel].size} total)"
#    urls = $link_store[m].map { |l| "http://#{l[:url]} posted by #{l[:nick]}" } || []
#    urls.uniq.reverse[0..2].each { |url| msg channel, url }
#  else
#    msg channel, "No URLs yet!"
#  end
#end
 on :message, /.*(sergey).*/i do |m|
	 sergeyleave = Time.new(2013,07,26)
	 nowtime = Time.now
	 difftime = sergeyleave.yday - nowtime.yday
	 m.reply "Time til Sergey Zero: #{difftime} days"
  end 
